...
import BuscarNoPertenecientesUAEM from "@/models/no-perteneciente-uaem/components/buscar";
import TablaDatosNoPertenecientesUAEMRegistrados from "@/models/no-perteneciente-uaem/components/tablaDatos";

const NuevoAccesoNoPertenecientesUAEM = async ({
  searchParams,
}: {
  searchParams?: {
    query?: string;
  ...
  return (
    <div className="flex justify-center flex-col items-center">
      ...
      <div className="overflow-x-auto flex flex-col justify-center items-center text-center py-2 my-5">
        <BuscarNoPertenecientesUAEM />
        <Suspense key={query} fallback={<Spinner />}>
          <TablaDatosNoPertenecientesUAEMRegistrados query={query} />
        </Suspense>
      </div>
    </div>
  );
...
export default NuevoAccesoNoPertenecientesUAEM;
